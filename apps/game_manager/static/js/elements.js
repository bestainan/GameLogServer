jQuery(document).ready(function () {
    ...
    // just-post
    if (jQuery('.just-post').length > 0) {
        jQuery('.just-post').click
        (function () {
            jConfirm('确定添加吗？？', '添加物品', function (r) {
                if (r) {
                    var values = jQuery('.chzn-single').find('span');
                    var add_item_tid = values.html().split(' ')[0];
                    var add_item_uid = jQuery('#user_id').attr('value');
                    var add_server_id = jQuery('#server_id').attr('value');
                    var add_item_function_name = jQuery('.add_item_function_name').attr('value');
                    jQuery.ajax({
                        type: "post",
                        ...
                        data: {
                            ...
                            item_id: add_item_tid,
                            user_id: add_item_uid,
                            function_name: add_item_function_name,
                        },
                        success: function () {
                            location.reload()
                        },
    ...
    // del-post
    if (jQuery('.del-post').length > 0) {
        jQuery('.del-post').click
        (function () {
            var values = jQuery(this).attr('value')
            jConfirm('确定删除吗？？', '删除物品', function (r) {
                if (r) {
                    var server_id = values.split('##')[0]
                    var item_id = values.split('##')[1]
                    var user_id = jQuery('#user_id').attr('value');
                    var del_item_function_name = jQuery('.del_item_function_name').attr('value');
                    jQuery.ajax({
                        type: "post",
                        url: "/Tyranitar6/post/data/",
                        dataType: 'json',
                        ...
                        data: {
                            ...
                            item_id: item_id,
                            user_id: user_id,
                            function_name: del_item_function_name,
                        },
                        success: function () {
                            location.reload()
                        },
    ...
    //检查是否是纯数字
    if (jQuery('.checknum').length > 0) {
        $('.checknum').onkeyup(function () {
           jAlert('1`23')
        });
        //jQuery('.checknum').onafterpaste(jQuery(this).val(jQuery(this).val.replace(/\D/g,''))
    };
