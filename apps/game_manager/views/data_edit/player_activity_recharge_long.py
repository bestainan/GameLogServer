...
def get_player_activity_recharge_long_function(request, templates):
    """
        玩家运营数据编辑 长期充值
    ...
    # 以函数的指针方式传递函数并调用
    function_name = 'data_edit.player_activity_recharge_long.{function}'.format(function=set_player_activity_recharge_long_function.__name__)
    ...
    if request.method == 'POST':
        ...
        if cmem_url:
            try:
                source = dict()
                if len(user_uid):
                    source = memcache.get_cmem_val(cmem_url, model_define.PLAYER_ACTIVITY_RECHARGE_LONG_MODEL.format(user_id=user_uid))
                elif len(user_name):
                ...
                if source:
                    ...
                    try:
                        ...
                        reward_str = ''                                                                 # 字符串
                        cur_reward_config_lst = game_config.get_all_reward_time_recharge_long_config()  # 策划填的配置表
                        if reward_lst:
