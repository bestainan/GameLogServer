...
def get_player_activity_hard_stage_function(request, player_activity_hard_stage):
    """
        玩家困难运营数据编辑
    ...
    """
    # 以函数的指针方式传递函数并调用
    function_name = 'data_edit.PlayerActivityHardStage.{function}'.format(function=set_player_activity_hard_stage_function.__name__)
    ...
    if request.method == 'POST':
        ...
        if cmem_url:
            try:
                source = dict()
                if len(user_uid):
                    source = memcache.get_cmem_val(cmem_url, model_define.PLAYER_ACTIVITY_HARD_STAGE_MODEL.format(user_id=user_uid))
                elif len(user_name):
                ...
                if source:
                    ...
                    try:
                        ...
                        reward_str = ''                                                                 # 字符串
                        cur_reward_config_lst = game_config.get_all_reward_times_hard_stage_config()  # 策划填的配置表
                        if reward_lst:
                    ...
                    if hard_win_count != 'None':
                        row_dict['hard_win_count'] = {'name': u'困难副本战胜次数', 'num': hard_win_count}
                    # -----------------------------------------------------------------------------------------------------------------------#
                else:
                    if user_uid:
                        return_uid = user_uid
                ...
                return render_to_response(player_activity_hard_stage, locals(), RequestContext(request))
...
def set_player_activity_hard_stage_function(value_dict):
    ...
    if cmem_url:
        if len(value_dict['user_id']):
            source = memcache.get_cmem_val(cmem_url, model_define.PLAYER_ACTIVITY_HARD_STAGE_MODEL.format(user_id=value_dict['user_id']))
            try:
                key = value_dict['item_id']
                if int(value_dict['input_value']) >= 0 and 'hard_win_count' == key:
            ...
            result = memcache.put_cmem_val(cmem_url, model_define.PLAYER_ACTIVITY_HARD_STAGE_MODEL.format(user_id=value_dict['user_id']), source)
            return result