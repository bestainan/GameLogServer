...
def get_player_reward_seven_power_info(request):
    """
        玩家开服七天战力
    """
    ...
    if request.method == 'POST':
        ...
        if cmem_url:
                ...
                source = {}
                if len(user_uid):
                    source = memcache.get_cmem_val(cmem_url, model_define.PLAYER_REWARD_SEVEN_POWER_MODEL.format(user_id=user_uid))
                elif len(user_name):
                ...
                if source:
                    ...
                    new_reward_config_lst = game_config.get_all_reward_seven_power_config()
        ...
        return render_to_response("data_edit/player_reward_seven_power_info.html",locals(),RequestContext(request))
    else:
        return render_to_response("data_edit/player_reward_seven_power_info.html",locals(),RequestContext(request))