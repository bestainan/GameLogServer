...
# templates player_activity_stone_consumption.html
def get_player_activity_stone_consumption_function(request, templates):
    """
        活动玩家消耗钻石次数
    ...
    """
    # 以函数的指针方式传递函数并调用
    function_name = 'data_edit.player_activity_stone_consumption.{function}'.format(function=set_player_activity_stone_consumption_function.__name__)
    ...
    if request.method == 'POST':
        ...
        if cmem_url:
            try:
                source = dict()
                if len(user_uid):
                    source = memcache.get_cmem_val(cmem_url, model_define.PLAYER_ACTIVITY_STONE_CONSUMPTION_MODEL.format(user_id=user_uid))
                elif len(user_name):
                ...
                if source:
                    ...
                    try:
                        ...
                        reward_str = ''                                                                 # 字符串
                        cur_reward_config_lst = game_config.get_all_reward_stone_consumption_config()  # 策划填的配置表
                        if reward_lst:
                    ...
                    # 道馆战胜次数
                    stone_consumption_win_count = source.get('num', 'None')
                    if stone_consumption_win_count != 'None':
                        row_dict['stone_consumption_win_count'] = {'name': u'消耗钻石数量', 'num': stone_consumption_win_count}
                    # -----------------------------------------------------------------------------------------------------------------------#
                else:
                    if user_uid:
                        return_uid = user_uid
...
# @require_permission
def set_player_activity_stone_consumption_function(value_dict):
    """
    修改memcache数据

    value_dict['tem_id'] stone_consumption_win_count
    value_dict['input_value'] int
    """
    cmem_url = server_define.CMEM_MAP[int(value_dict['server_id'])]
    if cmem_url:
        if len(value_dict['user_id']):
            source = memcache.get_cmem_val(cmem_url, model_define.PLAYER_ACTIVITY_STONE_CONSUMPTION_MODEL.format(user_id=value_dict['user_id']))
            try:
                key = value_dict['item_id']
                if int(value_dict['input_value']) >= 0 and 'stone_consumption_win_count' == key:
            ...
            result = memcache.put_cmem_val(cmem_url, model_define.PLAYER_ACTIVITY_STONE_CONSUMPTION_MODEL.format(user_id=value_dict['user_id']), source)
            return result