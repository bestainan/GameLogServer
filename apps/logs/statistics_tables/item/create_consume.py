...
def get_create_table(search_start_date, search_end_date, server_id):
    new_log_lst = dat_log_util.read_file_with_filename("CREATE_ITEM",search_start_date,search_end_date,server_id,'tables')
...
def get_consume_table(search_start_date, search_end_date,server_id):
    #获取搜索区间日志
    new_log_lst = dat_log_util.read_file_with_filename("CONSUME_ITEM",search_start_date,search_end_date,server_id,'tables')
    search_monster_log_list = []
    create_monster_action = set()
    head_name_lst = []
