...
def get_table(search_start_date, search_end_date, channel_id=-1, server_id=-1):
    ...
    for _day in xrange(total_days):
        ...
        for shop_id in xrange(1, 9):
            user_num = mysql_util.get_first_recharge_shop_index_uid_num('uid', 'EVENT_ACTION_RECHARGE_PLAYER'+str(date_str), row_date, 'shop_index', shop_id, channel_id, server_id)
            # 插入数据
            row.append(user_num)
